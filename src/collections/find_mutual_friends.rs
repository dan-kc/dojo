// Social Network Mutual Friends Practice
//
// Learning objectives:
// - Use HashSet for social network algorithms
// - Find intersection of friend sets efficiently
// - Handle missing data in HashMap lookups
//
// Run with: cargo test find_mutual_friends
...
mod tests {
    ...
    fn test_find_mutual_friends_multiple_mutual() {
        ...
        friendships.insert("Alice".to_string(), 
            ["Bob", "Charlie", "David", "Eve"].iter().map(|s| s.to_string()).collect());
        friendships.insert("Frank".to_string(), 
            ["Bob", "Charlie", "Grace", "Henry"].iter().map(|s| s.to_string()).collect());
    ...
    fn test_find_mutual_friends_asymmetric_friendship() {
        ...
        friendships.insert("Alice".to_string(), 
            ["Bob", "Charlie", "David"].iter().map(|s| s.to_string()).collect());
        friendships.insert("Bob".to_string(), 
            ["Charlie", "Eve", "Frank"].iter().map(|s| s.to_string()).collect());
    ...
    fn test_find_mutual_friends_single_mutual() {
        ...
        friendships.insert("Alice".to_string(), 
            ["SharedFriend", "AliceUnique1", "AliceUnique2"].iter().map(|s| s.to_string()).collect());
        friendships.insert("Bob".to_string(), 
            ["SharedFriend", "BobUnique1", "BobUnique2"].iter().map(|s| s.to_string()).collect());
    ...
    fn test_find_mutual_friends_circular_relationships() {
        ...
        friendships.insert("A".to_string(), 
            ["B", "C"].iter().map(|s| s.to_string()).collect());
        friendships.insert("B".to_string(), 
            ["A", "C"].iter().map(|s| s.to_string()).collect());
        friendships.insert("C".to_string(), 
    ...
    fn test_find_mutual_friends_order_independence() {
        ...
        friendships.insert("Alice".to_string(), 
            ["Friend1", "Friend2", "Friend3"].iter().map(|s| s.to_string()).collect());
        friendships.insert("Bob".to_string(), 
            ["Friend2", "Friend3", "Friend4"].iter().map(|s| s.to_string()).collect());
        ...
        let expected: HashSet<String> = ["Friend2", "Friend3"].iter().map(|s| s.to_string()).collect();
