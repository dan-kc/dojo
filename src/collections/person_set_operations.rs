// Custom HashSet with Person Type Practice
//
// Learning objectives:
// - Implement custom Hash trait for complex types
// - Use HashSet with custom hashable objects
// - Understand email-based equality for Person objects
//
// Run with: cargo test person_set_operations
...
mod tests {
    ...
    fn test_person_contains_lookup() {
        ...
        let lookup_person = Person::new("Different Name".to_string(), 99, "test@email.com".to_string());
        assert!(set.contains(&lookup_person));
        
        // Should not find person with different email
        let different_person = Person::new("Test".to_string(), 20, "different@email.com".to_string());
