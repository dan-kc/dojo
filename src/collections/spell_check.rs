...
mod tests {
    ...
    fn test_spell_check_case_sensitive() {
        ...
        let expected_errors: HashSet<String> =
            ["hello", "world"].iter().map(|s| s.to_string()).collect();
    ...
    fn test_spell_check_punctuation() {
        ...
        assert!(
            errors.is_empty()
                || errors.contains("hello,")
                || errors.contains("world!")
                || errors.contains("test?")
        );
    }
    ...
    fn test_spell_check_repeated_words() {
        let dictionary: HashSet<String> =
            ["correct", "word"].iter().map(|s| s.to_string()).collect();
        ...
        //  left: {"correctwrongcorrectwrongword"}
        // right: {"wrong"}
    }
